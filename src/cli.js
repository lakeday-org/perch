...
import { createModel, DEFAULT_MODEL, EFFORTS } from './model.js';
...
const options = {
  ...
  effort: ['--effort E', `Reasoning effort for every OpenAI call: ${EFFORTS.join(', ')}. By default attempt 1 gets none and each rejected attempt buys one step more`, ['fix', 'refactor']],
...
const commandHelp = {
  ...
  fix: { args: '[path]', summary: 'Fix open defects in this checkout, one commit per fix, each checked by the tests that reach the method and by System One', detail: 'Works the open defects, most likely first, up to --budget of them; with a path, only the defects in that file or directory. Each one is first put to System One again: can a real caller reach the flagged line given the method\'s own guards? If not, the finding is closed and no generative call is spent. Otherwise an OpenAI model sees everything System One answered about the method (every defect kind with its probability, the line, severity, which calls and callers look wrong) with the same neighborhood the hunt used, and returns the corrected method and nothing else. The method is spliced in by line range and must parse without adding nesting, more than one branch, or more than a point of risk. The tests that already reach the method must still pass in this checkout. System One then asks the hunt\'s questions again over the patched method: the defect and its kind must look less likely than before, no caller newly misused, nothing changed beyond the defect. A fix that passes is committed on the current branch with the summary as its message; a rejected attempt leaves the checkout as it was. Three attempts, each fed the previous rejection, with more reasoning effort each time. Refuses to run on main or master. A finding id in place of the path works one finding. Needs OPENAI_API_KEY and TYPESAFE_API_KEY.' },
