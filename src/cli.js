...
Environment:
${column([['TYPESAFE_API_KEY', 'scan, check'], ['PERCH_BASE_URL', 'scan, check: API base URL (default https://api.typesafe.ai/v1)']])}`;
...
const commands = {
  async scan(io) {
    ...
    const systemOne = metered(createSystemOne({ apiKey: io.env.TYPESAFE_API_KEY, baseUrl: io.env.PERCH_BASE_URL, log: retrying }), meter);
  ...
  async check(io) {
    ...
    const meter = createMeter();
    const systemOne = metered(createSystemOne({ apiKey: io.env.TYPESAFE_API_KEY, baseUrl: io.env.PERCH_BASE_URL, log: io.debug }), meter);
    const root = await repoRoot(process.cwd());
