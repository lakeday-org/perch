 ...
 */
export function huntAnswers(finding) {
  ...
  const lines = [
    `reachable behavioral defect: ${pct(finding.has_bug)}`,
...
export function fixPrompt({ finding, before, fileMetrics = null, budget = null, state, region, start, end, checks = [] }) {
  ...
  const room = budget ? `THE FILE MAY NOT GET MUCH BIGGER: measure rejects a rewrite that leaves the file above ${round(budget.sloc)} lines${Number.isFinite(budget.cyclomatic_complexity) ? ` or complexity ${round(budget.cyclomatic_complexity)}` : ', and there is no complexity ceiling here because closing a hole costs branches'}. Extracting helpers costs a few lines and that is allowed; if your rewrite is much longer than what it replaces, you have moved the mess rather than removed it. Delete what the split makes redundant.` : '';
...
OBJECTIVES:
${objectives}
HOW THEY ARE JUDGED: rescan runs the same scan over your rewrite (tree-sitter metrics and System One, same callers and callees). It adds up how many problems each reading expects, counting every answer at the probability the model gives it, and keeps correctness and design apart. Neither total may go up and at least one must come down. There is no threshold to clear: halving a defect's probability counts for exactly that, and an issue the scan names for the first time counts against you only as much as it is believed. Tests that reach the method${checks.length ? ` (${checks.join(', ')})` : ''} must still pass.
${room}
