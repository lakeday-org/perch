...
export function fixPrompt({ finding, before, reachable = null, fileMetrics = null, budget = null, state, region, start, end, checks = [] }) {
  ...
  return `Rewrite so every issue below is resolved. The tools are yours to use as you see fit: read anything in the checkout you need, then measure, rescan, and run_tests until they all pass, then submit. submit refuses anything the three have not passed. When a tool rejects a version, change the source and go again; do not resubmit the same one, and do not stop to explain — keep working until it passes or you have nothing left to try.
