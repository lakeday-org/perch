...
export function refactorPrompt({ node, metrics, state, region, start, end }) {
  return `Make one method simpler without changing what it does, using the tools: measure every version you write, run_tests and verify_with_system_one the one that measures better, then submit it. submit refuses anything the three have not passed. When a tool rejects a version, read its reason and change the source; do not resubmit it.
