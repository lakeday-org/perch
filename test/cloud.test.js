...
describe('Perch Cloud', () => {
  ...
  it('an explicit key still wins over the Actions token', async () => {
    const env = { GITHUB_ACTIONS: 'true', ACTIONS_ID_TOKEN_REQUEST_URL: 'https://actions.example/token', ACTIONS_ID_TOKEN_REQUEST_TOKEN: 'runtime', PERCH_API_KEY: 'key', XDG_CONFIG_HOME: '/nonexistent' };
    expect((await configuredSystemOne({ env, root: process.cwd(), fetchImpl: async () => { throw new Error('no request expected'); } })).report).toBeUndefined();
  });
