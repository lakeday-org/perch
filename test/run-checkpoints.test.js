...
it('keeps only the current run, since a run folder holds every answer it got', async () => {
  const { store } = await fixture();
  for (const name of ['earlier', 'current']) await mkdir(store.runDir(name), { recursive: true });
  await store.prune('runs', 'current');
  expect(await readdir(join(store.out, 'runs'))).toEqual(['current']);
});
